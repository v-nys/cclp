...
@(gen-graph-illustration-eval '(require graph pict pretty-graphs))

@title{Recursion analysis using a genealogical graph}
@defmodule[cclp/genealogical-graph]

A genealogical graph is an auxiliary data structure used to detect similarity between abstract conjunctions, where one is not simply a renaming or an instance of the other. More precisely, if conjunctions which display this similarity appear during a computation, a multi abstraction is required in order to complete the program analysis.

@emph{A genealogical graph is derived from a single branch of the abstract analysis tree.} @margin-note{It is easy to overlook this aspect, as many genealogical graphs are simply trees, which can lead to confusion.}
...
@margin-note{TODO: give the partial order?}
...
The analyses of these three conjunctions essentially consist of the same steps. Some of those steps are simply repeated more often in the longer conjunctions.
...
@examples[#:eval gen-graph-illustration-eval
          (dag->pict
           (directed-graph '((1 2) (1 3) (3 4)))
           (λ (vert) (text (number->string vert))))]
