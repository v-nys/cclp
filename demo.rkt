...
(define segments (segments-for-postprocessing ((compose sort-segments set->list synthesizable-segments analysis-tree) complete-analysis) (analysis-tree complete-analysis)))
(define endpoints (map (λ (l) (cons (label-index (first l)) (last l))) segments))
(for-each
 displayln
 endpoints)
;(tree-display  print-tree-label)
